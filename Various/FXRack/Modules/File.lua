...
--[[
==================================
Функции собраны в кучу не от фонаря. Большинство дублирует имеющиеся, но это не тупость!
Лучше, когда находятся в сборе и с соотв. именами.
Писать каждый раз с нуля или искать задалбывает.
Остальное по файлам - тоже добавлять сюда.
Копирование можно сделать через os.execute(), но системнозависмо и косячило.
Через read/write работает везде и всегда, но намного медленнее. Размер ограничен(ок.900M).
Зато надежно, в сети пишут то же.
==================================
Кроме этого, можно сюда добавить маленький локальный буфер обмена<ToClipboard>.
Ну, и в бетах SWS есть copy to cliboard, дождаться релиза, потестить.
Если что - поменять ф-и. Главное имена не менять!!!
Короче, пока будет так.
--]]
